...
import {
	BOUNTY_STATUS,
	canReclaimExpiredBounty,
	getReclaimErrorReason,
} from "@/lib/bounty-state";
...
function BountyListItem({
	...
	const canReclaim = canReclaimExpiredBounty(
		bounty,
		connectedAddress,
		nowSeconds,
	);
	...
	const showReclaim =
		isPoster && (bounty.status === BOUNTY_STATUS.Expired || isExpired);
	const reclaimReason = getReclaimErrorReason(
		bounty,
		nowSeconds,
		connectedAddress,
	);
	...
	const config = statusConfig[bounty.status as keyof typeof statusConfig] || {
		label: "Unknown",
		variant: "default" as const,
	};
				...
				<div className="flex flex-wrap gap-x-6 gap-y-1 font-mono text-xs text-ink-muted">
					...
					<span>
						{bounty.status === BOUNTY_STATUS.Open
							? isExpired
								? "Expired"
								: `Expires in ${Math.ceil((Number(bounty.expiresAt) - nowSeconds) / 3600)}h`
							: `Status: ${config.label}`}
					</span>
				</div>
			</div>
			...
			<div className="flex items-center gap-3">
				...
				{showReclaim && (
					<Button
						...
						disabled={!canReclaim}
						title={!canReclaim ? reclaimReason || "Cannot reclaim" : undefined}
					>
						Reclaim ETH
					</Button>
				)}
...
export default function MyPostsPage() {
		...
		return (
			<div className="space-y-4">
				{[1, 2, 3].map((i) => (
					<div
						key={i}
						className="h-32 w-full animate-pulse bg-surface-raised border-2 border-border"
					/>
				))}
			</div>
		);
	...
	if (error) {
		return (
			<div className="p-8 border-2 border-error bg-surface-raised">
				<h3 className="font-display text-xl text-error mb-2">
	...
	const openPosts = bounties.filter(
		(b) => b.status === BOUNTY_STATUS.Open && Number(b.expiresAt) >= nowSeconds,
	);
	const paidPosts = bounties.filter((b) => b.status === BOUNTY_STATUS.Paid);
	const expiredPosts = bounties.filter(
		(b) =>
			b.status === BOUNTY_STATUS.Expired ||
			(b.status === BOUNTY_STATUS.Open && Number(b.expiresAt) < nowSeconds),
	);
